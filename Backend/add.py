...
def get_db_connection():
    conn = mysql.connector.connect(
        ...
        database='py_fertilizer'  # Name of your database
    )
    return conn
...
def Custloginquery():
    if request.method == 'POST':
        ...
        if customer:
            ...
            flash('Login successful!', 'success')
            return redirect(url_for('sales'))  # Redirect to product.html page
        else:
            flash('Invalid username or password', 'danger')
...
def cart_customer():
    ...
    if request.method == 'POST':
        try:
            ...
            cursor.execute("""
                INSERT INTO sale_transaction 
                (bill_date,Customer_Name, Customer_Mobile, Customer_Address, Grand_Total, Pay_Mode)
                VALUES (%s,%s, %s, %s, %s, %s)
            """, (bill_date,customer_name, customer_mobile, customer_address, gtotal, pay_mode))
            conn.commit()
